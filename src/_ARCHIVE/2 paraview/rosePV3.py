...
def export_horn_torus_ply(resolution=100, filename="horn_torus.ply"):
    logging.info("Exporting horn torus to PLY file for ParaView.")
    X, Y, Z, colors = generate_3d_horn_torus(resolution)

    # Flatten the arrays for PyVista
    points = np.c_[X.ravel(), Y.ravel(), Z.ravel()]
    mesh = pv.PolyData(points)

    # Create RGBA colors using pyvista.Color for each point
    rgba_colors = []
    for i in range(colors.shape[0]):
        for j in range(colors.shape[1]):
            r, g, b, a = colors[i, j]
            rgba_colors.append(pv.Color([r, g, b], opacity=a).int_rgba)

    # Convert the list of RGBA values into a NumPy array
    rgba_colors = np.array(rgba_colors, dtype=np.uint8)

    # Assign the RGBA colors to the mesh
    mesh.point_data['RGBA'] = rgba_colors

    # Write the mesh to a PLY file with the RGBA texture
    mesh.save(filename, texture='RGBA', binary=True)
    logging.info(f"Horn torus exported successfully to {filename}.")

if __name__ == "__main__":
    render_horn_torus_3d(resolution=100, even_density=True, camera_position=(45, 60))
    export_horn_torus_ply(resolution=100, filename="horn_torus.ply")
