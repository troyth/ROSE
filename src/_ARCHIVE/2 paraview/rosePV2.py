...
def export_horn_torus_vtp(resolution=100, filename="horn_torus.vtp"):
    logging.info("Exporting horn torus to VTP file for ParaView.")
    ...
    # Add the RGB colors as a point array
    rgb_colors = (colors[:, :, :3] * 255).astype(np.uint8).reshape(-1, 3)
    mesh['Colors'] = rgb_colors

    # Add opacity as a scalar array
    opacity_values = colors[:, :, 3].ravel()  # Flatten the alpha channel array
    mesh['Opacity'] = opacity_values
...
if __name__ == "__main__":
    ...
    export_horn_torus_vtp(resolution=100, filename="horn_torus2.vtp")
