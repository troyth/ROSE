...
def generate_3d_horn_torus(resolution=100, even_density=False):
    ...
    Opacity = (V / (2 * np.pi))  # Normalized opacity from 0 to 1
...
def render_horn_torus_3d(resolution=100, even_density=False, view_elevation=True):
    ...
    ax.set_axis_off()
    if view_elevation:
        ax.view_init(elev=0, azim=0)  # Set the view to get a true side view
...
if __name__ == "__main__":
    render_horn_torus_3d(resolution=100, even_density=True, view_elevation=True)
