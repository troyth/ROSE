...
# Define the color space within the circle of radius 100
# Colors beyond the circle are not plotted
mask = R <= 100
...
xyz_min = np.min(xyz, axis=(0, 1))
xyz_max = np.max(xyz, axis=(0, 1))
...
# Plot the circle boundary with radius 100
circle = plt.Circle((0, 0), 100, color='black', fill=False, linewidth=1.5)
...
plt.title('Developmental Color Space (U, Gaia) using CIELChuv at L* = 50')
